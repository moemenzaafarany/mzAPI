...
{
    //===============================================================================//
    public static function scan(string $path, bool $deep_scan = false, bool $get_size = false): object
    {
        try {
            $path = realpath($path);
            $array = [];
            ...
            if (!is_dir($path)) {
                ...
                return mzAPI::return(200, null, null, $array);
            }
            //
            ...
            return mzAPI::return(500, "scan_failed=$e");
        }
    }
    //===============================================================================//
    ...
    //===============================================================================//
    public static function file(string $path, bool $create = false): object
    {
        try {
            if (!is_file($path)) {
                $dir = dirname($path);
            ...
            return mzAPI::return(200, null, null, realpath($path));
        } catch (Exception $e) {
            if ($new == true) rmdir($dir);
            return mzAPI::return(500, "file_error=$e");
        }
    }
    //===============================================================================//
    public static function folder(string $path, bool $create = false): object
    {
        try {
            if (!is_dir($path)) {
                if ($create == true) {
            ...
            return mzAPI::return(200, realpath($path));
        } catch (Exception $e) {
            return mzAPI::return(500, "folder_error=$e");
        }
    }
    //===============================================================================//
    public static function delete(string $path): object
    {
        try {
            if (is_file($path)) {
                if (unlink($path)) return mzAPI::return(200);
                throw new Exception("cannot_delete_file ('$path')");
            }
            if (is_dir($path)) {
                $array = mzFiles::recurringDelete($path);
                if (empty($array)) return mzAPI::return(200);
                throw new Exception("cannot_delete_files ('" . implode("','", $array) . "')");
            }
            return mzAPI::return(400, null, "path_doesnot_exist", null);
        } catch (Exception $e) {
            return mzAPI::return(500, "delete_failed=$e");
        }
    }
    //===============================================================================//
    ...
    //===============================================================================//
    public static function rename(string $path, string $name): object
    {
        try {
            if (file_exists($path)) {
                $path = realpath($path);
                ...
                if (rename($path, $rename)) return mzAPI::return(200, null, null, $rename);
                else throw new Exception("cannot_rename_path");
            }
            return mzAPI::return(400, null, "path_doesnot_exist", null);
        } catch (Exception $e) {
            return mzAPI::return(500, "rename_failed=$e");
        }
    }
    //===============================================================================//
    public static function move(string $path, string $move_path, bool $replace = false): object
    {
        try {
            $move = null;
            if (file_exists($path) && is_dir($move_path)) {
                ...
                if (rename($path, $move)) {
                    mzFiles::delete($path);
                    return mzAPI::return(200, null, null, $move);
                } else throw new Exception("cannot_rename_path");
            }
            return mzAPI::return(400, null, "path_doesnot_exist", null);
        } catch (Exception $e) {
            if ($move !== null && file_exists($move)) mzFiles::delete($move);
            return mzAPI::return(500, "move_failed=$e");
        }
    }
    //===============================================================================//
    public static function copy(string $path, string $copy_path, bool $replace = false): object
    {
        try {
            $copy = null;
            if (file_exists($path) && file_exists($copy_path)) {
                ...
                if (is_file($path)) {
                    if (copy($path, $copy_path)) return mzAPI::return(200, null, null, $copy);
                ...
                if (is_dir($path)) {
                    ...
                    if (empty($array)) return mzAPI::return(200, null, null, $copy);
            ...
            return mzAPI::return(500, "copy_failed=$e");
        }
    }
    //===============================================================================//
    public static function compress(string $path, string $output_path = "", string $type = "application/zip"): object
    {
        try {
            ...
            if ($type == "application/zip" && class_exists("ZipArchive")) {
                ...
                return mzAPI::return(200, null, null, ["$output.zip"]);
            }
            //
            throw new Exception("file_type_unsupported");
        } catch (Exception $e) {
            if ($new == true) mzFiles::delete($output_path);
            return mzAPI::return(500, "compression_failed=$e");
        }
    }
    //===============================================================================//
    public static function extract(string $path, string $output_path = "", bool $replace = false): object
    {
        if (!is_file($path)) return mzAPI::return(400, null, "path_doesnot_exist");
        try {
            $path = realpath($path);
            ...
            if ($file['mime'] == "application/zip" && class_exists("ZipArchive")) {
                ...
                $archive->close();
                return mzAPI::return(200, null, null, $output);
            }
            ...
            if ($new == true) mzFiles::delete($output_path);
            return mzAPI::return(500, "extraction_failed=$e");
        }
    }
}
