...
class mzParams
{
    ...
    {
        if (!empty($name)) return @$_SERVER[$name];
        return @$_SERVER;
    }
    //============================================================// headers
    ...
    //============================================================// UploadFile
    public static function addUploadFile(string $name, $value = null, bool $is_required = false, bool $is_uploaded = false, bool $exclude = false, bool $is_array = false, array $check_for_ext = null, int $max_size_in_kilobytes = null,  int $array_min_items = null, int $array_max_items = null)
    {
        // upload value
        ...
        if ($is_array == true) {
            ...
            if (!empty(mzParams::$params[$name])) {
                foreach (mzParams::$params[$name] as $i => $v) {
                    $rs = mzParams::_checkUploadFile($v, $is_required, $check_for_ext, $max_size_in_kilobytes);
                    if ($rs->status != 200) {
                        mzParams::$errors[$name] = [];
                        return mzParams::$errors[$name][$i] = $rs->error;
                    }
            ...
            $rs = mzParams::_checkUploadFile(mzParams::$params[$name], $is_required, $check_for_ext, $max_size_in_kilobytes);
            if ($rs->status != 200) return mzParams::$errors[$name] = $rs->error;
            mzParams::$params[$name] = $rs->data;
        }
    }
    //============================================================// UploadFile
    public static function addEquation(string $name, $value = null, bool $is_required = false, bool $is_uploaded = false, bool $exclude = false, bool $is_array = false, int $min_length = null, int $max_length = null, int $array_min_items = null, int $array_max_items = null)
    {
        // upload value
        if ($is_uploaded == true && !array_key_exists($name, mzParams::inputs()) && $exclude == true) return;
        if ($is_uploaded == true) mzParams::$params[$name] = mzParams::inputs($name);
        ...
        if ($is_array == true) {
            ...
            if (!empty(mzParams::$params[$name])) {
                foreach (mzParams::$params[$name] as $i => $v) {
                    $rs = mzParams::_checkEquation($v, $is_required, $min_length, $max_length);
                    if ($rs->status != 200) {
                        mzParams::$errors[$name] = [];
                        return mzParams::$errors[$name][$i] = $rs->error;
                    }
            ...
            $rs = mzParams::_checkEquation(mzParams::$params[$name], $is_required, $min_length, $max_length);
            if ($rs->status != 200) return mzParams::$errors[$name] = $rs->error;
            mzParams::$params[$name] = $rs->data;
        }
    }
    ...
    public static function getToken(string $name, int $length, int $array_length = null)
    {
        if (!empty($array_length)) {
            ...
            for ($i = 0; $i < $array_length; $i++) {
                $rs = mzParams::_createToken($length);
                if ($rs->status != 200) {
                    mzParams::$errors[$name] = [];
                    return mzParams::$errors[$name][$i] = $rs->error;
                }
    ...
    public static function uploadFile(string $name, string $path = null, string $filename = null, bool $media = true)
    {
        ...
        //
        if ($media) {
            if (!empty($path)) $path = mzAPI::MEDIA_DIR .  $path;
            else $path = mzAPI::MEDIA_DIR;
        }
        //
        if (empty($files)) return;
        if ($files === array()) {
            foreach (mzParams::$params[$name] as $i => $file) {
                // upload value
    ...
    {
        $equations = mzParams::params($name);
        //
        if (empty($equations)) return;
        if ($equations === array()) {
            foreach (mzParams::$params[$name] as $i => $equation) {
                // upload value
                if (empty(@mzParams::$params[$name][$i])) return mzParams::$errors[$name][$i] = "equation_invalid";
                //
                foreach ($variables as $k => $v) {
                    $equation = str_replace(strtolower($k), $v, strtolower($equation));
                }
                //
                mzParams::$params[$name][$i] = eval("return $equation;");
                return;
            }
            return;
        } else {
            $equation = $equations;
            // upload value
            if (empty(@mzParams::$params[$name])) return mzParams::$errors[$name] = "equation_invalid";
            //
            foreach ($variables as $k => $v) {
                $equation = str_replace(strtolower($k), $v, strtolower($equation));
            }
            //
            mzParams::$params[$name] = eval("return $equation;");
            return;
        }
    }
    //===============================================================================// Validators
    ...
    {
        if ($is_required == true && empty($value)) return new mzRes(400, "required");
        if ($is_required == false && empty($value)) return new mzRes(200);
        if ((!empty($value) || $value == 0) && !is_array($value)) return new mzRes(400, "invalid_array");
        if (!empty($array_min_items) && Count($value) < $array_min_items) return new mzRes(400, "min_items=" . $array_min_items);
        if (!empty($array_max_items) && Count($value) > $array_max_items) return new mzRes(400, "max_items=" . $array_max_items);
        return new mzRes(200, null, null, $value);
    }
    //============================================================// Int
    private static function _checkInt($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if (is_array($value)) return new mzRes(400, "invalid_int");
        ...
        // valid
        if (!is_numeric($value)) return new mzRes(400, "invalid_int");
        $value = intval($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
        // return
        return new mzRes(200, null, null, $value);
    }
    //============================================================// Float
    private static function _checkFloat($value = null, bool $is_required = false, int $min_length = null, int $max_length = null, array $format_options = null): mzRes
    {
        // _check if required and empty
        if (is_array($value)) return new mzRes(400, "invalid_float");
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) || strlen($value) == 0)) return new mzRes(200);
        // valid
        if (!is_numeric($value)) return new mzRes(400, "invalid_float");
    ...
    private static function _checkString($value = null, bool $is_required = false, int $min_length = null, int $max_length = null, array $format_options = null, array $_check_for_values = null): mzRes
    {
        ...
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        // valid
        if (!is_string($value)) return new mzRes(400, "invalid_string");
        $value = strVal($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// Domain
    private static function _checkDomain($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        // valid
        if (!checkdnsrr($value)) return new mzRes(400, "invalid_domain");
        $value = strVal($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// Coordinates
    private static function _checkCoordinates($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        ...
        if (preg_match("/^(?<latitude>[-]?[0-8]?[0-9]\.\d+|[-]?90\.0+?)(?<delimeter>.)(?<longitude>[-]?1[0-7][0-9]\.\d+|[-]?[0-9]?[0-9]\.\d+|[-]?180\.0+?)$/", $value) == 0) return new mzRes(400, "invalid_coordinates");
        $value = strVal($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// Telephone
    private static function _checkTelephone($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        ...
        if (preg_match("/^[+][0-9]/", $value) == 0 && preg_match("/^[0-9]/", $value) == 0) return new mzRes(400, "invalid_telephone");
        $value = strVal($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// Link
    private static function _checkLink($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        // valid
        if (!filter_var($value, FILTER_VALIDATE_URL))  return new mzRes(400, "invalid_url");
        $value = strVal($value);
        // length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// Timestamp
    private static function _checkTimestamp($value = null, bool $is_required = false, string $min_date = null, string $max_date = null, array $format_options = null): mzRes
    {
        // _check if required and empty
        ...
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
        // valid
        if (strtotime($value) == false)  return new mzRes(400, "invalid_timestamp");
        $value = strVal($value);
        // length
        if (!empty($min_date) && strtotime($min_date) != false && strtotime($value) < strtotime($min_date)) return new mzRes(400, "min_date=" . $min_date);
        if (!empty($max_date)  && strtotime($max_date) != false && strtotime($value) > strtotime($min_date)) return new mzRes(400, "max_date=" . $max_date);
        // format
    ...
    //============================================================// Color
    private static function _checkColor($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        ...
        ) return new mzRes(400, "invalid_color");
        $value = strVal($value);
        // _check length
        if (!empty($min_length) && strlen($value) < $min_length) return new mzRes(400, "min_length=" . $min_length);
        if (!empty($max_length) && strlen($value) > $max_length) return new mzRes(400, "max_length=" . $max_length);
    ...
    //============================================================// UploadFile
    private static function _checkUploadFile($file = null, bool $is_required = false, array $check_for_ext = null, int $max_size_in_kilobytes = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (!file_exists(@$file['tmp_name']) || !is_uploaded_file(@$file['tmp_name']))) return new mzRes(400, "required");
        if ($is_required == false && (!file_exists(@$file['tmp_name']) || !is_uploaded_file(@$file['tmp_name']))) return new mzRes(200);
        // valid
        if (UPLOAD_ERR_OK !== @$file["error"]) return new mzRes(400, "file_error");
        if (!empty($check_for_ext) && !in_array(strtolower(pathinfo(@$file["name"], PATHINFO_EXTENSION)), array_map('strtolower', $check_for_ext))) return new mzRes(400, "file_error");
        if (!empty($max_size_in_kilobytes) && @$file["size"] > ($max_size_in_kilobytes * 1000)) return new mzRes(400, "max_file_size=$max_size_in_kilobytes");
        // return
        return new mzRes(200, null, null, $file);
    }
    //============================================================// Link
    private static function _checkEquation($value = null, bool $is_required = false, int $min_length = null, int $max_length = null): mzRes
    {
        // _check if required and empty
        if ($is_required == true && (empty($value) && strlen($value) == 0)) return new mzRes(400, "required");
        if ($is_required == false && (empty($value) && strlen($value) == 0)) return new mzRes(200);
    ...
    //============================================================// Token
    private static function _createToken(int $length): mzRes
    {
        // create
    ...
    public static function params(string $parameter = null, array $format_options = null, array &$array_push = null)
    {
        if (!empty($parameter)) {
            ...
            //return
            if (isset($array_push)) {
                if (array_key_exists($keys[0], mzParams::$params)) $array_push[$name] = $val;
                return $array_push;
            }
            return $val;
        }
