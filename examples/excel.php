<?php
ini_set("sys_temp_dir", "./tmp/system");
ini_set("upload_tmp_dir", "./tmp/upload");
putenv('TMPDIR=./tmp/dir');


echo ini_get("upload_tmp_dir");

/*
define("_APPNAME", "mzAPI");
...
$excel = [
    ...
    <Application>' . _APPNAME . '</Application></Properties>',
    ...
    <fileVersion appName="' . _APPNAME . '"/><sheets>
    {SHEETS}
    </sheets></workbook>',
...
class mzExcel
{
            ...
            if ($cfilename === 'xl/_rels/workbook.xml.rels') {
                ...
                foreach ($this->F as $f) {
                    $FONTS[] = '<font><name val="' . $this->defaultFont . '"/><family val="2"/>'
                        ...
                        . ($f & self::F_BLUE ? '<color rgb="FF0B6AB0"/>' : '')
                        . ($f & self::F_BLACK ? '<color rgb="FF000000"/>' : '')
                        . ($f & self::F_WHITE ? '<color rgb="FFFFFFFF"/>' : '')
                        . '</font>';
                }
                $FONTS[] = '</fonts>';
                $XF = ['<cellXfs count="' . count($this->XF) . '">'];
                foreach ($this->XF as $xf) {
                    $align = ($xf[2] === self::A_LEFT ? ' applyAlignment="1"><alignment horizontal="left" vertical="center"/>' : '')
                        . ($xf[2] === self::A_RIGHT ? ' applyAlignment="1"><alignment horizontal="right" vertical="center"/>' : '')
                        . ($xf[2] === self::A_CENTER ? ' applyAlignment="1"><alignment horizontal="center" vertical="center"/>' : '');
    ...
    protected function _sheetToXML($idx, $template)
    {
        ...
        if (count($this->sheets[$idx]['rows'])) {
            ...
            foreach ($this->sheets[$idx]['rows'] as $r) {
                $CUR_ROW++;
                $row = '<row r="' . $CUR_ROW . '" autoFitHeight="0" height="2.75">';
                $CUR_COL = 0;
                foreach ($r as $v) {
                    $CUR_COL++;
                    ...
                    if (is_string($v)) {
                        ...
                        if (true) {
                            if (strpos($v, '<') !== false) { // tags?
                                ...
                                if (strpos($v, '<blue>') !== false) {
                                    $F += self::F_BLUE;
                                }
                                if (strpos($v, '<black>') !== false) {
                                    $F += self::F_BLACK;
                                }
                                if (strpos($v, '<white>') !== false) {
                                    $F += self::F_WHITE;
                                }
                                ...
                                if (preg_match('/<rowspan="([^"]+)">/i', $v, $m)) {
                                    $span = intVal($m[1]) - 1;
                                    if (!empty($span) & $span > 0) {
                                        if (!isset($MERGES[$cname])) $MERGES[$cname] = [];
                                        if (!isset($MERGES[$cname]['c'])) $MERGES[$cname]['c'] = $CUR_COL;
                                        $MERGES[$cname]['r'] = $CUR_ROW + $span;
                                    }
                                }
                                if (preg_match('/<colspan="([^"]+)">/i', $v, $m)) {
                                    $span = intVal($m[1]) - 1;
                                    if (!empty($span) & $span > 0) {
                                        if (!isset($MERGES[$cname])) $MERGES[$cname] = [];
                                        if (!isset($MERGES[$cname]['r'])) $MERGES[$cname]['r'] = $CUR_ROW;
                                        $MERGES[$cname]['c'] = $CUR_COL + $span;
                                    }
                                }
                                if (preg_match('/<color="([^"]+)">/i', $v, $m)) {
                                    $value = $m[1];
                                }
                                if (preg_match('/<hAlign="([^"]+)">/i', $v, $m)) {
                                    $value = $m[1];
                                }
                                if (preg_match('/<vAlign="([^"]+)">/i', $v, $m)) {
                                    $value = $m[1];
                                }
                                $v = strip_tags($v);
                            } // tags
            ...
            foreach ($COL as $k => $max) {
                ...
                $width = 5;
                if ($max > 5) $width = 12.5;
                if (isset($this->colsWidth[$k])) $width = $this->colsWidth[$k];
                $COLS[] = '<col min="' . $k . '" max="' . $k . '" width="' . $width . '" />';
            }
        ...
        $MERGECELLS = "";
        if (!empty($MERGES)) {
            $MERGECELLS = '<mergeCells count="' . count($MERGES) . '">' . "\n";
            foreach ($MERGES as $key => $value) {
                $cell2 = $this->num2name($value['c']) . $value['r'];
                if ($cell2) $MERGECELLS .= '<mergeCell ref="' . $key . ":" . $cell2 . '"/>' . "\n";
            }
    ...
    public function date2excel($year, $month, $day, $hours = 0, $minutes = 0, $seconds = 0)
    {
        ...
        //    Calculate the Julian Date, then subtract the mzExcel base date (JD 2415020 = 31-Dec-1899 Giving mzExcel Date of 0)
        $century = substr($year, 0, 2);
        $decade = substr($year, 2, 2);
        $excelDate = floor((146097 * $century) / 4) + floor((1461 * $decade) / 4) + floor((153 * $month + 2) / 5) + $day + 1721119 - $mymzExcelBaseDate + $excel1900isLeapYear;
